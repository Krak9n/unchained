...
ft::ft() {
  if (ft::initialize("extra/fonts/tight_pixel.ttf")) {
    std::cerr << "Font error!\n";
  }
...
int ft::initialize(std::string path) {
  ...
  if (FT_New_Face(font.ft, path.c_str(), 0, &font.face)) {
    std::cerr << "Error FreeType: couldn't create new face, or failed to load font." 
      << std::endl;
    return -1;
  }
  ...
  if (FT_Load_Char(font.face, 'f', FT_LOAD_RENDER)) {
    std::cerr << "Erorr: couldn't load glyph." << std::endl;
    return -1;
  } 
...
int ft::loop() {
  ...
  for (unsigned char c = 0; c < 128; ++c) {
    if (FT_Load_Char(font.face, c, FT_LOAD_RENDER)) {
      std::cout << "Error: couldn't load glyph." << std::endl;
      return -1;
    }
